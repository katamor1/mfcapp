...
// 複数のFind／Has呼出しをまたぐ原子的Snapshotや、外部副作用まで有効な予約Lockは提供しない。
// TimePointは同じ単調IClock系列から供給し、壁時計の変更に依存させないこと。
// 例外契約: Record Containerの資源割当失敗はResultへ変換する保証がなく、
// 回復可能な操作拒否ではなくProcess資源異常として扱う。
...
public:
    ...
    // StoreはcompletedAtがstartedAt以後かを検証しないため、同じ非逆行Clock系列を渡すこと。
    ...
    // StoreはcompletedAtがstartedAt以後かを検証しない。
    ...
    // 戻り値だけでは後続の外部要求まで競合が増えないことを保証しない。
    ...
    // nowがstartedAtより前の場合は経過済みと扱わず、Errorも返さない。
